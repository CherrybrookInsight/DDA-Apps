...
PS.SUM_VOLUME_STATUS = 'hypervolemic' or RS.REFERRAL_SOURCE_CHRONIC_STABLE = 'ACute Pulmonary Oedema') THEN 'Yes' else 'No' END VolumeOverload
...
left join HF_Patient_Referral_Sources RS on A.HT_PAT_ID = RS.HT_PAT_ID and A.VISIT_ID = RS.VISIT_ID
where
(case when ASMNT_INV_ECHOCARDIOGRAPHY_LVEF >= '50.0' then ASMNT_INV_ECHOCARDIOGRAPHY_LVEF
	 when ASMNT_INV_ECHOCARDIOGRAPHY_LVEF = '' then case when ASMNT_INV_LOWEST_LVEF >= '50.0' then ASMNT_INV_LOWEST_LVEF end
end) >= '50.0'
...
drop table #temp
select * from #temp
where  (diff <= 365 or diff is null) and baseline = 1